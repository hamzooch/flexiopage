  ...
  if (!wallet) {
    return (
      <div className="hidden items-center gap-1.5 sm:flex">
        <span className="h-9 w-24 animate-pulse rounded-xl bg-muted/40" />
        <span className="h-9 w-24 animate-pulse rounded-xl bg-muted/40" />
      </div>
    );
  }
    ...
    <div className="hidden items-center gap-1.5 lg:flex">
      <Link
        ...
        className={`group inline-flex h-10 items-center gap-2 rounded-xl border px-3 transition-colors ${
          lowMain ? 'border-amber-500/40 bg-amber-500/5 hover:bg-amber-500/10' : 'border-border/60 bg-card hover:bg-muted'
        }`}
        title="Solde principal — débité 3% par commande livrée"
      >
        <span className={`grid h-6 w-6 place-items-center rounded-md ${lowMain ? 'bg-amber-500/20 text-amber-700' : 'bg-emerald-500/15 text-emerald-700'}`}>
          <WalletIcon className="h-3.5 w-3.5" />
        </span>
        <span className="flex flex-col leading-none">
          <span className="text-[9px] font-semibold uppercase tracking-wider text-muted-foreground">Solde</span>
          <span className="mt-0.5 text-sm font-bold">{fmt(wallet.balance, wallet.currency)}</span>
        </span>
      </Link>
      ...
      <Link
        ...
        className={`group inline-flex h-10 items-center gap-2 rounded-xl border px-3 transition-colors ${
          lowAi ? 'border-amber-500/40 bg-amber-500/5 hover:bg-amber-500/10' : 'border-border/60 bg-card hover:bg-muted'
        }`}
        title="Solde IA — débité à chaque génération de landing / page produit"
      >
        <span className={`grid h-6 w-6 place-items-center rounded-md ${lowAi ? 'bg-amber-500/20 text-amber-700' : 'bg-fuchsia-500/15 text-fuchsia-700'}`}>
