...
type Step = 'idle' | 'choose-type' | 'choose-theme' | 'create-form';
...
interface Props {
  /** Called after a successful creation. If not given, navigates to the new store hub. */
  onCreated?: (storeId: string) => void;
  /** Optional CTA label override for the trigger button when idle. */
  triggerLabel?: string;
}
          ...
          {TYPE_OPTIONS.map((opt) => {
            ...
            return (
              <button
                key={opt.id}
                type="button"
                onClick={() => { setSelectedType(opt.id); setStep('choose-theme'); }}
                className={cn(
