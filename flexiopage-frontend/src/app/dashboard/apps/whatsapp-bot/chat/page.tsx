...
/**
 * Interface de discussions plein écran (type WhatsApp Web) pour le WhatsApp Bot.
 * Le vendeur voit ses conversations en quasi temps réel (polling), peut
 * reprendre la main (takeover → le bot se tait) et répondre manuellement.
 *
 * Réutilise les endpoints messenger-bot avec channel=whatsapp via whatsappBotApi.
 */

import { useCallback, useEffect, useMemo, useRef, useState } from 'react';
import Link from 'next/link';
import { useSearchParams } from 'next/navigation';
import { ArrowLeft, Loader2, Search, Send, RefreshCw, Hand, ShoppingBag, MessageSquare } from 'lucide-react';
import { whatsappBotApi, extractApiError, type MessengerConversation, type MessengerMessage } from '@/lib/api';
...
export default function WhatsAppChatPage() {
  ...
  const refreshList = useCallback(async (silent = false) => {
    ...
    try {
      const res = await whatsappBotApi.listConversations(storeId, { limit: 50 });
  ...
  const loadThread = useCallback(async (id: string, silent = false) => {
    ...
    try {
      ...
      // Ne remplace que si la conversation sélectionnée n'a pas changé entre-temps.
  ...
  // Chargement initial + polling de la liste.
  ...
  async function takeover() {
    if (!sel) return;
    try {
      await whatsappBotApi.takeover(storeId, sel);
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <Link href={`/dashboard/apps/whatsapp-bot?storeId=${encodeURIComponent(storeId)}`}
          ...
          <div className="min-w-0">
            <h1 className="text-base font-semibold leading-tight sm:text-lg">Discussions WhatsApp</h1>
